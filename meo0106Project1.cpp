...
int main()
{
	//for randomly generated seeded variables
    	srand(time(NULL));
	...
	//STEP 4 -ask the user for the player's name
	...
	for(int i=0;i<name.length()-1;++i)
	{
    	...
    	//convert uppercase letters, that are not supposed to, to lowercase letters
    	/*for(int i=0;i<name.length()-1;++i)
    	{
	...
	while(second<=first) //if second is less than first integer//which it can not be
	{
    	...
    	//STEP 6: Placeholders for the random numbers if the user asks for them
    	int display1=-1,display2=-1;
    	//cout<<display1<<"\t"<<display2<<endl;

    
    	do //STEP 9 -  loop to allow the user to repeat steps 7 and 8
    	{
            ...
            //-----------------------------------------------------------------------------------------------------
					...
					if(display1>-1 || display2>-1) // if user displayed one side
					{
                    			...
                    			else // if the user did not display anything -1	-1
                    			{
                            			if(guess>first && guess<second)
                    				{
	...
	//-----------------------------------------------------------------------------------------------------
    	...
    	}while(points>=0); //STEP 9 -  loop to allow the user to repeat steps 7 and 8
